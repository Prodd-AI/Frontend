...
import { MdOutlineCenterFocusStrong, MdOutlineRemoveRedEye } from 'react-icons/md';
...
const PersonalTabsSection = ({
	...
	const primaryGoalDescription = (todaysTasks as UserTaskAssignment[]).find(
		(userTask) => userTask.task.priority === 'high' && userTask.task.status === 'pending',
	)?.task.description;
				...
				},
				{
					...
					content: (
						<MoodTrends
							...
							averageMood={data?.data.average_mood ? MoodEntryMapper[data.data.average_mood] : undefined}
						/>
					),
				},
				{
