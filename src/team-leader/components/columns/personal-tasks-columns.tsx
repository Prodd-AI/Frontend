...
// Each priority gets its own color so the table doesn't blur medium and low
// into the same neutral pill. Matches the priority tiers used in Today's Focus.
...
// Due dates use wall-clock-as-UTC encoding from the API, so parse them
// without applying the local offset (matches Today's Focus rendering).
...
export const personalTasksColumns: ColumnDef<UserTaskAssignment>[] = [
  ...
  },
  {
    ...
    header: "",
    cell: ({ row }) => (
      <TaskActionsCell task={row.original.task} canRequestChanges={false} />
    ),
  },
];
